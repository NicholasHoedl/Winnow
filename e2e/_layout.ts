...
/**
 * Every screen, at a given width, measured rather than eyeballed.
 *
 ...
 * why both are here.
 *
 * **This file was extracted from `mobile-layout.spec.ts` so a second sweep could reuse it,
 * and that extraction is the point.** The detector had been aimed at 393px alone for its
 * whole life, which is why every spill at 1280px went unseen — a clipped currency figure in
 * the dashboard's budget tile among them. One detector, two widths, one place to fix the
 * next blind spot.
 */
 ...
 * guess at them.
 */
export async function layoutFaults(page: Page): Promise<Fault[]> {
 ...
 * and an empty test account would never produce one on its own. It earns its place at
 * desktop widths too — the clipping found at 1280px was in the dashboard's budget and macro
 * tiles, which is precisely where a larger figure presses.
 *
 * Created through the UI rather than the database, deliberately, so it is shaped exactly as
 * the app shapes it. **Teardown is the other way round** — see `clearWideContent`.
 *
 * Each caller passes its own `prefix`, so the two sweeps seed and sweep independently rather
 * than sharing a row and having to agree about who removes it.
 */
export async function seedWideContent(
  browser: Browser,
  prefix: string,
): Promise<void> {
...
/**
 * Remove it again, in SQL.
 *
 * The seed goes through the UI and the teardown does not, and that asymmetry is the house
 * rule rather than an inconsistency: a delete that is the ASSERTION stays in the UI, and
 * this one is cleanup. The UI version this replaced opened a row menu on `/budget`, which
 * made every layout run depend on the budget page rendering a specific row — a page this
