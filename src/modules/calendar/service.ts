...
export type ExceptionOverlay = {
  ...
  endAt: Date | string | null
  allDay: boolean | null
  /** Null inherits the series' flag; `false` untracks this one date of a series. */
...
export function applyExceptions<E extends OverlayableEvent>(
  ...
  for (const occ of occurrences) {
    ...
    const effective = {
      ...
      // `??`, so an override of `false` untracks this one date rather than falling back to
      // a tracked series. That asymmetry is the reason the column is nullable.
      tracked: ex.tracked ?? series.tracked,
    } as E
