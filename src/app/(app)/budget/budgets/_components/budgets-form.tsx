...
/**
 * The month's limits as a form on the page: a total, and one amount per expense
 * category. The body of the "Set budgets" dialog until T30, when the section split into
 * pages and the dialog's body became this page's, the way the Lists and Repeating tasks
 * dialogs became pages in T25.
 *
 * No Cancel and nothing to close: the page is the editor. It re-seeds itself whenever the
 * month or the saved figures change — after Save, after Copy last month, after a month
 * change — which is what a dialog did by reopening.
 */
export function BudgetsForm({
  ...
  /** categoryId → currently budgeted minor units for this month. Memoized by the caller. */
  ...
  // One row per expense category, seeded from this month's budgets — and re-seeded when
  // they change under the form, which is how a save or a copy shows its result.
  React.useEffect(() => {
    reset({
      total:
  ...
  }, [month, categories, budgetedByCategory, monthlyBudgetCents, currency, reset])
  ...
  // almost certainly a slip — but not one this form should refuse to save. `useWatch`
  ...
  return (
    <section className="mt-6">
      <h2 className="text-sm font-semibold">Set budgets</h2>
      <p className="text-muted-foreground mt-1 text-sm">
        A total for the month, and a limit per expense category if you want
        one. Leave a field blank for no limit.
      </p>

      <form onSubmit={onSubmit} className="mt-3 rounded-xl border p-4">
        {/* The one thing about the total that a field on one month's form cannot show is
            that it stands for the months after it — so the hint says so. */}
        ...
        <p className="text-muted-foreground mt-1.5 text-xs">
          Everything you spend, measured against one figure. It stays in effect
          for the months after this one until you change it.
        </p>
        {overTotal && (
